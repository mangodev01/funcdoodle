...
namespace FuncDoodle::Themes {
	...
	struct CustomTheme {
		...
		bool OwnsMeta =
			false;	///< Whether Name and Author memory must be freed.
		CustomTheme() : Name(""), Author("") {}
		/**
	...
	inline CustomTheme* LoadThemeFromFile(const char* path) {
		...
		if (!result) {
			FUNC_WARN(
				"Failed to parse theme file... it's probably not valid TOML");
		...
		if (table.contains("other"sv) && table.get("other"sv)->is_table()) {
			...
			read_float(
				"WindowBorderHoverPadding"sv, style.WindowBorderHoverPadding);
			read_vec2("WindowMinSize"sv, style.WindowMinSize);
			read_vec2("WindowTitleAlign"sv, style.WindowTitleAlign);
			read_enum(
				"WindowMenuButtonPosition"sv, style.WindowMenuButtonPosition);
			...
			read_float(
				"DragDropTargetBorderSize"sv, style.DragDropTargetBorderSize);
			read_float("DragDropTargetPadding"sv, style.DragDropTargetPadding);
			...
			read_vec2("DisplaySafeAreaPadding"sv, style.DisplaySafeAreaPadding);
			read_bool(
				"DockingNodeHasCloseButton"sv, style.DockingNodeHasCloseButton);
			read_float("DockingSeparatorSize"sv, style.DockingSeparatorSize);
			read_float("MouseCursorScale"sv, style.MouseCursorScale);
			read_bool("AntiAliasedLines"sv, style.AntiAliasedLines);
			read_bool("AntiAliasedLinesUseTex"sv, style.AntiAliasedLinesUseTex);
			read_bool("AntiAliasedFill"sv, style.AntiAliasedFill);
			read_float("CurveTessellationTol"sv, style.CurveTessellationTol);
		...
		if (std::filesystem::exists(path) &&
			...
			for (const std::filesystem::directory_entry& e :
				...
				if (!e.is_regular_file()) {
					FUNC_GRAY("Skipping {} because it isn't a regular file",
						e.path().string());
					continue;
				}
			...
			if (ImGui::Button("Save")) {
...

				{
					...
					for (unsigned char i = 0; i < ImGuiCol_COUNT; ++i) {
						...
						colors.insert(iv,
							toml::array{style.Colors[i].x, style.Colors[i].y,
